 ...
 */
export function loadRequire<T>(
  id: string
): CatchReturnType<ILoaderValue<T>, LoaderRequireError> {
  try {
 ...
 */
export async function loadImport<T>(
  id: string
): Promise<CatchReturnType<ILoaderValue<T>, LoaderImportError>> {
  try {
 ...
 */
export async function loadJiti<T>(
  id: string,
  _filename: string,
  jitiOptions: JITIOptions = {},
  parentModule?: typeof module,
  parentCache?: Record<string, typeof module>
): Promise<CatchReturnType<ILoaderValue<T>, LoaderJitiError>> {
  try {
    const value = await jiti(
      __filename,
      jitiOptions,
      parentModule,
      parentCache
...
export async function loadWebpack(
  ...
  regExp?: RegExp
): Promise<CatchReturnType<IWebpackLoaderValue, LoaderWebpackError>> {
  try {
    ...
    return {
      ...
      data: {
        ...
        loadAll: () => {
          ...
          for (const key of keys) result[key] = context(key);
          return result;
        },
      },
    };
